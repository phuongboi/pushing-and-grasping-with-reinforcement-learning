...
class DQN:
    ...
    def agent_policy(self, state, epsilon):
        ...
        if np.random.rand() < epsilon:
            ...
            action_location_idx = np.random.randint(self.num_action_positions*self.num_action_positions)
    ...
    def train_with_relay_buffer(self):
        ...
        next_q_mat_orient, next_q_mat_loc = self.model(next_input_images, next_input_depths)
        ...
        next_q_vec_orient = np.max(next_q_mat_orient.detach().numpy(), axis=1).squeeze()
        next_q_vec_loc = np.max(next_q_mat_loc.detach().numpy(), axis=(1,2)).squeeze()
        #print("next_q_vec" ,next_q_vec.shape)
        target_vec_orient = rewards + self.gamma * next_q_vec_orient* (1 - terminals)
        target_vec_loc = rewards + self.gamma * next_q_vec_loc* (1 - terminals)
        q_mat_orient, q_mat_loc = self.model(input_images, input_depths)
        #q_mat = q_mat_loc.permute(0, 2, 3, 1)
        q_mat_loc = q_mat_loc.reshape(q_mat_loc.shape[0], q_mat_loc.shape[1]*q_mat_loc.shape[2])

        q_vec_orient = q_mat_orient.gather(dim=1, index=actions[:, 1].unsqueeze(1)).type(torch.FloatTensor)
        q_vec_loc = q_mat_loc.gather(dim=1, index=actions[:, 0].unsqueeze(1)).type(torch.FloatTensor)
        #print(q_vec.shape)
        target_vec_orient = torch.from_numpy(target_vec_orient).unsqueeze(1).type(torch.FloatTensor)
        target_vec_loc = torch.from_numpy(target_vec_loc).unsqueeze(1).type(torch.FloatTensor)
        orient_loss = self.loss_func(q_vec_orient, target_vec_orient)
        loc_loss = self.loss_func(q_vec_loc, target_vec_loc)
        loss = orient_loss + loc_loss
